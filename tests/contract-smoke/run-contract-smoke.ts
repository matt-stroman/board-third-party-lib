...
const smokeUserPassword =
  (process.env.CONTRACT_SMOKE_USER_PASSWORD ?? "LocalDevOnly!234").trim() || "LocalDevOnly!234";
...
function buildRequestPlan(
  ...
  switch (`${route.method} ${route.path}`) {
    ...
    case "PUT /player/followed-studios/{studioId}":
    case "DELETE /player/followed-studios/{studioId}":
      return {
        path: `/player/followed-studios/${requireValue(context.managedStudioId, "managed studio id")}`,
        init: { method: route.method, headers: buildHeaders(token) },
      };
    ...
    case "GET /developer/titles/{titleId}/showcase-media":
      return {
        path: `/developer/titles/${requireValue(context.createdTitleId, "created title id")}/showcase-media`,
        init: { method: route.method, headers: buildHeaders(token) },
      };
    case "POST /developer/titles/{titleId}/showcase-media":
      return {
        ...
        init: {
          ...
          body: JSON.stringify({
            kind: "external_video",
            videoUrl: "https://www.youtube.com/watch?v=board-enthusiasts-smoke",
            altText: "Smoke showcase preview",
            displayOrder: 1,
          }),
        },
      };
    case "PUT /developer/titles/{titleId}/showcase-media/{showcaseMediaId}":
      return {
        path: `/developer/titles/${requireValue(context.createdTitleId, "created title id")}/showcase-media/${requireValue(context.showcaseMediaUnderTestId, "showcase media id")}`,
        init: {
          method: route.method,
          headers: buildHeaders(token, "application/json"),
          body: JSON.stringify({
            videoUrl: "https://www.youtube.com/watch?v=board-enthusiasts-smoke-updated",
            altText: "Updated smoke showcase preview",
            displayOrder: 2,
          }),
        },
      };
    case "POST /developer/titles/{titleId}/showcase-media/{showcaseMediaId}/image-upload": {
      const formData = new FormData();
      formData.set("media", new Blob(["smoke-image"], { type: "image/png" }), "smoke.png");
      return {
        path: `/developer/titles/${requireValue(context.createdTitleId, "created title id")}/showcase-media/${requireValue(context.showcaseMediaUnderTestId, "showcase media id")}/image-upload`,
        init: { method: route.method, headers: buildHeaders(token), body: formData },
      };
    }
    case "DELETE /developer/titles/{titleId}/showcase-media/{showcaseMediaId}":
      return {
        path: `/developer/titles/${requireValue(context.createdTitleId, "created title id")}/showcase-media/${requireValue(context.showcaseMediaUnderTestId, "showcase media id")}`,
        init: { method: route.method, headers: buildHeaders(token) },
      };
