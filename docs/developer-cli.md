...
- `staging` keeps the stricter authenticated smoke path and therefore still expects:
  - `DEPLOY_SMOKE_PLAYER_EMAIL`
  - `DEPLOY_SMOKE_DEVELOPER_EMAIL`
  - `DEPLOY_SMOKE_MODERATOR_EMAIL`
  - `DEPLOY_SMOKE_USER_PASSWORD`
- `production` can run with public-only post-deploy smoke when those dedicated smoke accounts are intentionally omitted
- if production later gets dedicated smoke accounts, the same full authenticated smoke path is reused automatically
...
- it creates or updates the Supabase Auth user
- it upserts the BE `app_users` projection
- it grants the roles needed for end-to-end access:
  - `player`
  - `developer`
  - `verified_developer`
  - `moderator`
