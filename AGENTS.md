...
- Frontend copy must be user-facing, friendly, and helpful. Write UI text as guidance for real users, not as developer-facing documentation of the interface.
- Frontend copy must be production-quality product copy that matches the tone of the rest of the site. Do not add explanatory text that merely narrates what is already obvious from the UI, calls out future implementation plans, or repeats instructions users can easily infer from the controls in front of them.
- Frontend validation and error text must appear directly beneath the input, button, or control they relate to so users encounter the guidance where they take the action. Do not surface actionable form feedback only in distant page-level banners.
- Frontend error messages must be understandable to non-technical users. Translate backend, database, and provider failures into plain-language product guidance instead of exposing raw constraint names, SQL fragments, or internal exception text whenever a friendly explanation is possible.
